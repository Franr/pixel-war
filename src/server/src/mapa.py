...
class Mapa:
    ...
    def __init__(self, nombre):
        ...
        sec = self.parse(Archivo(maps_folder, nombre + ".pwm").buffer)
        if sec:
    ...
    def gen_mapa_numerico(self, mapa: list[list[int]]):
    ...
    def get_id_by_pos(self, x: int, y: int) -> int:
        return self.array_map[y][x]

    def set_object(self, objeto, x: int, y: int):
        self.array_map[y][x] = objeto.get_uid()

    def set_red(self, x: int, y: int) -> None:
        self.x_rojo = x
        self.y_rojo = y

    def set_azul(self, x: int, y: int) -> None:
        self.x_azul = x
        self.y_azul = y
    ...
    def get_blue(self) -> tuple[int, int]:
    ...
    def pos_is_blocked(self, x: int, y: int) -> bool:
